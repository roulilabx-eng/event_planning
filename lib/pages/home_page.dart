...
import '../app.dart'; // ← 取用 AppLayout（全域 RWD）
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final double contentWidth = layout.contentWidth;
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            right: 0,
            child: Center(
              child: CountdownWidget(targetDateTime: eventTime),
            ),
          ),
          ...
          Positioned(
            ...
            child: SingleChildScrollView(
              padding: EdgeInsets.symmetric(
                horizontal: (layout.screenWidth - contentWidth) / 2,
              ),
              child: Column(
                children: [
                  // 標題
                    ...
                    builder: (context, constraints) {
                      ...
                      return AnimatedSwitcher(
                        ...
                        child: isStacked
                          ...
                          children: [
                            _buildLeftColumn(contentWidth, isStacked: true),
                            const SizedBox(height: 20),
                            // _buildRightColumn(contentWidth, isStacked: true),
                          ],
                        )
                            : Row(
                          ...
                          children: [
                            _buildLeftColumn(contentWidth),
                            const SizedBox(width: 20),
                            // _buildRightColumn(contentWidth),
                          ],
                        ),
                      );
                    },
  ...
  // 左側欄
  Widget _buildLeftColumn(double contentWidth, {bool isStacked = false}) {
    return SizedBox(
      width: isStacked ? double.infinity : contentWidth * 0.65,
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                const Text(' 🎄活動資訊 🎄', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                const Text('主題｜再不猜謎就瘋狂：身體小小、頭腦一級棒'),
                const Text('時間｜2025/12/24（三）19:00'),
                const Text('地點｜金色三麥'),
                const SizedBox(height: 20),
                const Text(' 🔔 活動須知 🔔 ', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                const Text('禮物金額｜300 元以上（不含包裝）'),
                GestureDetector(
                  onTap: _openSlotMachine,
                  child: Row(
                    children: const [
                      Text(
                        '禮物主題｜點擊右方聖誕樹',
                ...
                const SizedBox(height: 20),
                const Text(' 🎉 活動流程 🎉 ', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)),
                const SizedBox(height: 10),
                const Text('入場 & 報到（19:00–19:10）主辦人會收集每個人準備的謎題 & 禮物編號。'),
              ],
            ),
          ),
  ...
  // // 右側欄
  // Widget _buildRightColumn(double contentWidth, {bool isStacked = false}) {
  //   return SizedBox(
  //     width: isStacked ? double.infinity : contentWidth * 0.35,
